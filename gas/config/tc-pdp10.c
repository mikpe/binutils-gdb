...
{
  pdp10_number_to_chars_progbits ((char *) to, hword, 4);
}
...
pdp10_md_assemble (char *str)
{
  ...
  if (opnd.X_op != O_absent)
    {
      bfd_reloc_code_real_type r_type = BFD_RELOC_32; // FIXME: adjust for constant vs symbol ref
...
pdp10_md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
    {
    case BFD_RELOC_64:
      /* a 30-bit word address */
      if (value & 7)
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("Value %lu is not word-aligned."), value);
      if (value < -((1L << (30 + 3 - 1)) - 1) || value > (1L << (30 + 3 - 1)))
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("Value %ld out of 33-bit range."), value);
      fixP->fx_r_type = BFD_RELOC_PDP10_EFIW;
      pdp10_emit_word (where, (unsigned long) value >> 3);
      break;

    case BFD_RELOC_32:
      if (fixP->fx_addsy == NULL)	/* an 18-bit literal */
	{
	  if (value < -((1L << (18 - 1)) - 1) || value > (1L << (18 - 1)))
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _("Value %ld out of 18-bit range."), value);
	  pdp10_emit_hword (where, (unsigned long) value);
	}
      else				/* an 18-bit same-segment word address */
	{
	  if (value & 7)
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _("Value %lu is not word-aligned."), value);
	  if (value < -((1L << (18 + 3 - 1)) - 1) || value > (1L << (18 + 3 - 1)))
	    as_bad_where (fixP->fx_file, fixP->fx_line,
			  _("Value %ld out of 21-bit range."), value);
	  fixP->fx_r_type = BFD_RELOC_PDP10_IFIW;
	  pdp10_emit_hword (where, (unsigned long) value >> 3);
	}
      break;
